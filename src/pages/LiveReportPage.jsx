    ...
    <div className="space-y-4 h-[calc(100vh-6rem)] flex flex-col" data-testid="live-report-page">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 shrink-0">
        <PageHeader
          ...
          subtitle="Real-time 100% audit report detailing module coverage, API endpoints, schema verifications, and platform metrics."
        />
        <div className="flex items-center gap-2">
          <Button variant="outline" size="sm" onClick={handleRefresh}>
            <RefreshCw className="h-3.5 w-3.5 mr-1.5" /> Refresh Report
          </Button>
          <Button
            size="sm"
          ...
          >
            <ExternalLink className="h-3.5 w-3.5 mr-1.5" /> Open Fullscreen
          </Button>
        </div>
      </div>
      ...
      <div className="flex-1 w-full bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden min-h-[600px]">
        <iframe
          key={key}
          src="/report.html"
          title="JiNANAM Live Audit Report"
